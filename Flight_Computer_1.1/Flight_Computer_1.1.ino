//Second version of the Project Phobos flight computer, to be ran for the first time on 23/04/22
//written by Will Borden, 26/03/22, wborden@student.unimelb.edu.au
//This code reads values from sensors and records them to an array, stores important values on the EEPROM, then writes to SD card apon touchdown
...
/*Rocket States:
 ...
 ----------------------------------------------------------------------------------------------------------------------
    ...
    1   | ready; no guaranteed GPS lock        | GREEN   | 4 tones when ready, one beep every 4 seconds once GPS locked
...
int landing_alt = 3; //max altitude that counts as a landing
...
/*================================================================================*/
/*ARRAY, EEPROM AND SD CARD VARIABLES*/
double data_array[16384]; //volatile array that data is stored in
unsigned int address = 0;

int EEPROM_address = 0; //EEPROM address
int n = 0; //used for EEPROM when reading and writing
...
void setup() {
  ...
  //Serial.begin(115200);
  //Serial.println("Project Phobos flight software V1.1"); Serial.println("");
/*================================================================================*/
/*Pinmodes*/
...
/*sensor and SD initialisation*/
  ...
  if(bno.begin() && bmp.begin_I2C() && SD.begin(chipSelect)){
...
/*Liftoff and landing detection*/
  if(state == 1 && alt > launch_alt){
    //liftoff
    noTone(buzzerPin); //turn of buzzer in case it's on when landing is detected
    delay(100);
    state = 2;
  }
  if((state == 2 || state == 3) && alt < landing_alt){
    ...
    for(int i = 0; i <= 300; i++){
      ...
      data_array[address] = currentmillis / 1000;
      address++;
    ...
    String dataString = "";
    for(int i = 0; i < 16384; i=i+6){
      dataString = String(data_array[i],8) + "," + String(data_array[i+1],8) + "," + String(data_array[i+2],8) + "," + String(data_array[i+3],8) + "," + String(data_array[i+4],8) + "," + String(data_array[i+5],8);
...
/*================================================================================*/
/*begin writing to array and EEPROM, check if array is full*/
  if(state == 2){
    //write to array
    data_array[address] = currentmillis / 1000;
    address++;
    ...
    data_array[address] = gps_alt;
    address++;
    //write to EEPROM once per second
    if(currentmillis - EEPROM_previous >= 2000){
      EEPROM_previous = currentmillis;

      n = EEPROM_writeAnything(EEPROM_address*4,currentmillis);
      EEPROM_address++;
      n = EEPROM_writeAnything(EEPROM_address*4,latitude); //advance EEPROM address by 4 each time, as floats are written over 4 bytes
      EEPROM_address++;
      n = EEPROM_writeAnything(EEPROM_address*4,longitude);
      EEPROM_address++;
      n = EEPROM_writeAnything(EEPROM_address*4,acceleration_vertical);
      EEPROM_address++;
      n = EEPROM_writeAnything(EEPROM_address*4,alt);
      EEPROM_address++;
      n = EEPROM_writeAnything(EEPROM_address*4,gps_alt);
      EEPROM_address++;
    }
  ...
  if(address == 16383 || EEPROM_address == EEPROM.length()/4){ //check if array or EEPROM is full
    //stop writing to array and EEPROM
    state = 3; 
  }
/*================================================================================*/
/*Change LED colour*/
...
//functions to write floats to EEPROM, NOTE: variables written to EEPROM must be floats, not doubles
